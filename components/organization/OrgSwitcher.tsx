...
export const OrgSwitcher: React.FC = () => {
  ...
  const memberships = user?.member 
    ? (Array.isArray(user.member) ? user.member : [user.member])
    : [];
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-64 bg-gray-800 border-gray-700" align="start">
        ...
        {memberships.map((membership) => (
          <DropdownMenuItem
            ...
            onClick={() => {
              ...
              toast.success(`Switched to ${membership.org?.name || 'organization'}`);
            }}
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm text-white">{membership.org?.name || 'Organization'}</p>
